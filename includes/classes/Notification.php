...
class Notification {
    ...
    public function __construct($user_obj, $con) {
        $this->con = $con;
        $this->user_obj = new User($user_obj,$con);
    }

    public function getUnreadNumber() {
        $userLoggedIn = $this->user_obj->getUsername();

        // get all the inviewed notifucations for the logged n user
    ...
    public function insertNotification($post_id, $user_to, $type) {
        $userLoggedIn = $this->user_obj->getUsername();
        $userLoggedInFullName = $this->user_obj->getFirstAndLastName();
        $date_time = date("Y-m-d H:i:s");

        switch ($type){
            case 'comment':
                $message = $userLoggedInFullName . " commented on your post";
                break;
            case "like":
                $message = $userLoggedInFullName. " liked your post";
                break;
            case 'profile_post':
                $message = $userLoggedInFullName. " posted on your profile";
                break;
            case 'comment_non_owner':
                $message = $userLoggedInFullName ." commented on a post you commnetd on";
                break;
            case 'profile_comment':
                $message = $userLoggedInFullName. " commented on your profile post";
                break;
        }
        // send this over to post
